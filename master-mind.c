 ...
 * Compile:
 ...
 Run by passing secret
 sudo ./master-mind -s 123
...
void pinMode(uint32_t *gpio, int pin, int mode) {
  ...
  // translating this c line to assembly
  ...
  asm(
    ...
    "\t STR R2, [R0]\n"
    :
    :[gpio] "r" (gpio), [shift] "r" (shift), [fSel] "r" (fSel), [mode] "r" (mode)
    : "r0", "r1", "r2", "r3", "r4", "cc"
  );
}
...
void initSeq() {
  ...
  // Setting the seed to the time
  srand(time(NULL));

  // generating 3 random numbers 
  int random_num1 = (rand() % 3) + 1;
  int random_num2 = (rand() % 3) + 1;
  int random_num3 = (rand() % 3) + 1;

  // space for first sequence
  theSeq = calloc(3, sizeof(int));

  // secret sequence generation
  theSeq[0] = random_num1;
  theSeq[1] = random_num2;
  theSeq[2] = random_num3;
...
void showSeq(int *seq) {
  /* *  COMPLETE the code here  *  */
    ...
    // initializes an array to be used to store the different letters of a sequence
    char letters[3];

    for(int i = 0; i < 3; i++){
        switch (seq[i]){
            case 1: letters[i] = 'R'; break;
            case 2: letters[i] = 'G'; break;
            case 3: letters[i] = 'B'; break;
        }
    }
...
extern int matches(int* guess, int* sequence);
extern void showseq(int* sequence);
...
int countMatches(int *seq1, int *seq2) {
  int match = matches(seq1, seq2);
  return match;
}

/* show the results from calling countMatches on seq1 and seq1 */
void showMatches(int code, int *seq1, int *seq2, int i) {
  int exact = code/10;
  int close = code%10;
  printf("%d\n Exact matches", exact);
  printf("%d\n Approximate matches", close);
}
...
uint64_t timeInMicroseconds(){
  ...
  // Create a struct timeval to store the current time 
  struct timeval currentTime;
  // current time is extracted and stored in struct
  gettimeofday(&currentTime, NULL);
  // time in microseconds
  uint64_t microSec = ((uint64_t)currentTime.tv_sec * 1000000) + currentTime.tv_usec;
...
void timer_handler (int signum) {
  ...
  // time_out is set to 0 to indicate that the timer has expired */
  time_out = 0;
...
{ // this is just a suggestion of some variable that you may want to use
  ...
  pinMode(gpio, pinLED, 1);
  pinMode(gpio, pin2LED2, 1);
...
//	mode which sends the commands as 2 x 4-bit values.
  ...
  // Start of game
  lcdPosition (lcd, 0, 0) ;
  lcdPuts(lcd, "Welcome! press");
  lcdPosition (lcd, 0, 1) ;
  lcdPuts(lcd, "button to play");
  ...
  uint64_t startTime;
  uint64_t deltaTime; // cool fancy game terminology
  lcdClear(lcd);
  // -----------------------------------------------------------------------------
  // +++++ main loop
  while (!found) {
    ...
    for(int j = 0; j<3;j++){
      ...
      waitForButton(gpio, pinButton);
      clicks++;
      delayMicroseconds(400000); // cool down for button
      startTime = timeInMicroseconds();
      deltaTime = 0;
      while(deltaTime <= TIMEOUT){  // delay to confirm num of clicks
        if(readButton(gpio, pinButton) == 1){
          //fprintf(stdout, "button clicked\n");
          startTime = timeInMicroseconds();
          clicks++;
          delayMicroseconds(400000);// cool down for button
        }
        deltaTime = timeInMicroseconds() - startTime;
      }
      // flash red once 
      blinkN(gpio, pin2LED2, 1);

      // flash green LED X times
      blinkN(gpio, pinLED, clicks);

      attSeq[j] = clicks;
      clicks = 0;
      fprintf(stdout, "%d %d %d \n", attSeq[0],attSeq[1],attSeq[2]);
      lcdPosition(lcd, (j*2)+3 ,0);
      char a = attSeq[j]+'0';
      lcdPutchar(lcd, a);
      delayMicroseconds(500000);
    }
    ...
    // match from assembly
    int match = 0;
    match = matches(attSeq, theSeq);
    exact = match/10;
    contained = match%10;
    fprintf(stdout, "%d exact %d contained\n", exact, contained);
    

    // blink green exact amount
    blinkN(gpio, pinLED, exact);
    
    // Blink red to seperate 
    blinkN(gpio, pin2LED2, 1);

    // Blink green contained amount
